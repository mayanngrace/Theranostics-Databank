  ...
  <div id="app">
    ...
    <aside :class="{ 'nav-open': navVisible }" class="nav">
      <nav class="nav-menu">
        ...
        <router-link to="/home" @click="toHome()" class="nav-item"><i class="fas fa-file-upload">&nbsp&nbsp</i> New Record</router-link>
...
.nav-item {
  ...
  width: 250px; /* Set the width to 100% of its parent */
  padding: 15px 20px;
  box-sizing: border-box; /* Include padding and border in the element's width */
  text-align: left; /* Center the text horizontally */
  color: white; /* Text color */
  background-color: linear-gradient(to bottom , #441515 , rgb(98, 17, 17)); /* Deep maroon background */
  text-decoration: none;
...
.nav-open ~ .content {
  margin-left: 250px; /* Shift content when nav is open */
}
...
@media (max-width: 768px) {
  .nav {
    width: 200px;
    left: -200px; /* Adjust width for smaller screens */
  }

  .nav-open ~ .content {
    margin-left: 200px; /* Adjust content shift for smaller screens */
  }

  .toggle-button {
    padding: 10px; /* Smaller button on smaller screens */
  }
}
</style>
